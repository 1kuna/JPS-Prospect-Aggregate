...
class TestLLMProcessingAPI:
        ...
        with app.app_context():
            ...
            for i in range(num_prospects):
                ...
                prospect = Prospect(
                    ...
                    naics_source='original' if has_naics and random.random() > 0.7 else 'llm_inferred' if has_naics else None,
                    estimated_value_text=f'${random.randint(10, 500) * 1000:,}' if random.random() > 0.3 else 'TBD',
                    estimated_value_single=random.randint(10000, 1000000) if has_value else None,
                    title_enhanced=f'Enhanced Title {i}' if has_title_enhanced else None,
                    set_aside_standardized=random.choice(['SMALL_BUSINESS', 'EIGHT_A', 'WOSB', None]) if has_set_aside else None,
                    source_id=random.choice(sources).id,
                    loaded_at=datetime.now(timezone.utc) - timedelta(hours=random.randint(0, 168)),
                    ollama_processed_at=datetime.now(timezone.utc) if is_processed else None,
                    ollama_model_version='test-model-v1' if is_processed else None,
                    enhancement_status=random.choice(['idle', 'queued', 'processing', 'completed', 'failed'])
                )
        ...
        with patch('app.api.llm_processing.add_individual_enhancement') as mock_add:
            ...
            response = self._auth_request(
                ...
                json={
                    ...
                    'enhancement_type': random.choice(['all', 'values', 'titles', 'naics'])
                }
            )
    ...
    def test_get_queue_status(self, client):
        ...
        with patch('app.api.llm_processing.enhancement_queue') as mock_queue:
            ...
            mock_queue.get_queue_status.return_value = {
                ...
                'current_item': f'item-{random.randint(1, 100)}' if processing > 0 else None
            }
    ...
    def test_get_queue_item_status(self, client):
        ...
        with patch('app.api.llm_processing.enhancement_queue') as mock_queue:
            ...
            mock_queue.get_item_status.return_value = {
                'status': status,
                'queue_position': random.randint(1, 10) if status == 'pending' else None,
                'progress': {
            ...
            response = self._auth_request(client, 'get', f'/api/llm/queue/item/{item_id}')
    ...
    def test_get_queue_item_not_found(self, client):
        ...
        with patch('app.api.llm_processing.enhancement_queue') as mock_queue:
            ...
            response = self._auth_request(client, 'get', f'/api/llm/queue/item/nonexistent')
    ...
    def test_cancel_queue_item(self, client):
        ...
        with patch('app.api.llm_processing.enhancement_queue') as mock_queue:
            ...
            response = self._auth_request(client, 'post', f'/api/llm/queue/item/{item_id}/cancel')
    ...
    @pytest.mark.parametrize('enhancement_type', ['all', 'values', 'titles', 'naics', 'set_asides'])
