...
class TestDecisionsAPI:
    ...
    def test_decision_validation_behavior(self, client):
        ...
        invalid_requests = [
            ...
            {'prospect_id': random.choice(self.test_prospects).id, 'reason': 'Missing decision'},
            {'prospect_id': 'invalid-id', 'decision': 'go', 'reason': 'Invalid prospect'},
            {'prospect_id': random.choice(self.test_prospects).id, 'decision': 'maybe', 'reason': 'Invalid decision type'}
        ]
    ...
    def test_delete_nonexistent_decision(self, client):
        ...
        response = self._auth_request(
            client, 'delete', f'/api/decisions/{fake_id}'
        )
