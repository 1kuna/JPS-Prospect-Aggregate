...
class TestScraperConfig:
    ...
    def test_scraper_config_with_field_mappings(self):
        ...
        target_fields = ['contract_title', 'issuing_agency', 'estimated_value', 
                        'posted_date', 'opportunity_description', 'contact_info']
...
class TestConsolidatedScraperBase:
    ...
    def test_take_screenshot_success(self, scraper):
        ...
        before_screenshot = datetime.now(timezone.utc)
        screenshot_path = scraper._take_screenshot(mock_page, error_context)
        after_screenshot = datetime.now(timezone.utc)
    ...
    def test_save_html_dump_success(self, scraper):
        ...
        random_content = f"<html><body>Content-{random.randint(1000, 9999)}</body></html>"
    ...
    def test_extract_data_from_files_excel(self, scraper):
        ...
        try:
            ...
            test_data = {
                'contract_title': [f"Excel-Contract-{random.randint(1000, 9999)}" for _ in range(num_rows)],
                'issuing_agency': [f"Agency-{random.choice(['X', 'Y', 'Z'])}" for _ in range(num_rows)],
                'contract_value': [random.randint(50000, 500000) for _ in range(num_rows)],
                'posted_date': [f"2024-{random.randint(1,12):02d}-{random.randint(1,28):02d}" for _ in range(num_rows)]
            }
    ...
    def test_apply_field_mappings(self, scraper, test_config):
        ...
        for source_col, target_col in test_config.raw_column_rename_map.items():
            if 'title' in source_col.lower():
                original_data[source_col] = [f"Title-{random.randint(1000, 9999)}" for _ in range(num_rows)]
            elif 'agency' in source_col.lower():
                original_data[source_col] = [f"Agency-{random.choice(['A', 'B', 'C'])}" for _ in range(num_rows)]
            elif 'date' in source_col.lower():
                original_data[source_col] = [f"2024-{random.randint(1,12):02d}-{random.randint(1,28):02d}" for _ in range(num_rows)]
            else:
                original_data[source_col] = [f"Value-{random.randint(1, 1000)}" for _ in range(num_rows)]
    ...
    def test_clean_and_validate_data(self, scraper):
        ...
        for i in range(num_rows):
            ...
            test_data['agency'].append(f"Agency-{random.choice(['A', 'B', 'C'])}" if random.random() > 0.1 else '')
            test_data['description'].append(f"Desc-{i}" if random.random() > 0.3 else None)
            
            # Random date formats
            if random.random() > 0.3:
                test_data['posted_date'].append(f"2024-{random.randint(1,12):02d}-{random.randint(1,28):02d}")
            else:
                test_data['posted_date'].append(random.choice(['invalid-date', 'not-a-date', None]))
            ...
            test_data['estimated_value_text'].append(f"${random.randint(1000, 100000):,}" if random.random() > 0.3 else 'TBD')
    ...
    def test_create_prospects_from_dataframe(self, scraper, db_session):
        ...
        data_source = DataSource(
            ...
            last_scraped=datetime.now(timezone.utc) - timedelta(hours=random.randint(0, 48))
        )
        ...
        test_data = {
            'title': [f"Prospect-{random.randint(1000, 9999)}" for _ in range(num_prospects)],
            'description': [f"Desc-{random.randint(1, 100)}" for _ in range(num_prospects)],
            'agency': [source_name for _ in range(num_prospects)],
            'naics': [f"{random.randint(100000, 999999)}" for _ in range(num_prospects)],
            'posted_date': [f"2024-{random.randint(1,12):02d}-{random.randint(1,28):02d}" for _ in range(num_prospects)],
            'estimated_value_text': [f"${random.randint(10000, 1000000):,}" for _ in range(num_prospects)]
        }
    ...
    def test_save_prospects_to_database(self, scraper, db_session):
        ...
        for i in range(num_prospects):
            prospect = Prospect(
                ...
                loaded_at=datetime.now(timezone.utc) - timedelta(minutes=random.randint(0, 60))
            )
    ...
    def test_run_scraper_success(self, mock_playwright, scraper, db_session):
        ...
        with patch('app.core.consolidated_scraper_base.stealth_async'):
            ...
            for source_col in scraper.config.raw_column_rename_map.keys():
                test_data[source_col] = [f"Value-{random.randint(1000, 9999)}" for _ in range(num_rows)]
            
            # If no mappings, use default columns
            if not test_data:
                test_data = {
                    'title': [f"Contract-{random.randint(1000, 9999)}" for _ in range(num_rows)],
                    'agency': [f"Agency-{random.choice(['A', 'B'])}" for _ in range(num_rows)],
            ...
            with patch.object(scraper, '_extract_data_from_files') as mock_extract:
                ...
                with patch.object(scraper, '_save_prospects_to_database') as mock_save:
                    ...
                    mock_save.return_value = {'saved': saved_count, 'errors': error_count}
                    
                    with patch.object(scraper, 'scrape_data') as mock_scrape:
                        num_files = random.randint(1, 3)
                        mock_scrape.return_value = [Path(f'/fake/file{i}.csv') for i in range(num_files)]
    ...
    def test_scraper_custom_transformations(self, scraper):
        ...
        test_df = pd.DataFrame({
            'title': [f"Title-{random.randint(1000, 9999)}" for _ in range(num_rows)],
            'agency': [f"Agency-{random.choice(['X', 'Y', 'Z'])}" for _ in range(num_rows)],
    ...
    def test_scraper_file_cleanup(self, scraper):
        ...
        for i in range(num_files):
            ext = random.choice(extensions)
            if ext in ['.png', '.html']:
                dir_path = scraper.screenshots_dir if ext == '.png' else scraper.html_dumps_dir
            else:
    ...
    def test_scraper_configuration_validation(self):
        ...
        test_configs = [
            ...
            {'source_name': f"Config-{random.randint(1000, 9999)}", 'base_url': f"https://test{random.randint(1,100)}.gov"},
            {'source_name': f"Config-{random.randint(1000, 9999)}", 'navigation_timeout_ms': random.randint(10000, 60000)},
            {'source_name': f"Config-{random.randint(1000, 9999)}", 'screenshot_on_error': random.choice([True, False])}
        ]
