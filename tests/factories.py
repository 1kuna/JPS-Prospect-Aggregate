...
class ProspectFactory:
    ...
    def create(**kwargs) -> Dict[str, Any]:
        ...
        defaults = {
            ...
            "naics": (
                ProspectFactory.NAICS_CODES[idx % len(ProspectFactory.NAICS_CODES)]
                if idx % 4 != 0
                else None
            ),
            "naics_source": "original" if idx % 2 == 0 else "llm_inferred",
            "estimated_value_text": (
                f"${(idx + 1) * 10000:,}" if idx % 3 != 0 else "TBD"
            ),
            "estimated_value_single": (
                Decimal((idx + 1) * 10000) if idx % 3 != 0 else None
            ),
            "estimated_value_min": Decimal((idx + 1) * 9000) if idx % 3 != 0 else None,
            "estimated_value_max": Decimal((idx + 1) * 11000) if idx % 3 != 0 else None,
            "release_date": (datetime.now(UTC).date() - timedelta(days=idx)),
            "award_date": (datetime.now(UTC).date() + timedelta(days=30 - idx)),
            ...
            "place_country": "USA",
            "set_aside": ProspectFactory.SET_ASIDES[
                idx % len(ProspectFactory.SET_ASIDES)
            ],
            "set_aside_standardized": ["SMALL_BUSINESS", "EIGHT_A", "WOSB", None][
                idx % 4
            ],
            ...
            "enhancement_status": ["idle", "queued", "processing"][idx % 3],
            "extra": {},
        }
...
class DataSourceFactory:
    ...
    def create(**kwargs) -> Dict[str, Any]:
        ...
        defaults = {
            "name": f"{source_data[0]} {idx}",
            "url": source_data[1],
            "scraper_key": f"{source_data[2].lower()}-{idx}",
            "description": f"Scraper for {source_data[0]}",
...
class EnhancementQueueFactory:
    ...
    def create(**kwargs) -> Dict[str, Any]:
        ...
        defaults = {
            ...
            "user_id": (idx % 3) + 1,
            "enhancement_type": EnhancementQueueFactory.ENHANCEMENT_TYPES[
                idx % len(EnhancementQueueFactory.ENHANCEMENT_TYPES)
            ],
            "status": EnhancementQueueFactory.STATUSES[
                idx % len(EnhancementQueueFactory.STATUSES)
            ],
            "priority": idx % 3,  # 0=low, 1=medium, 2=high
            "created_at": datetime.now(UTC) - timedelta(minutes=idx * 10),
            "started_at": (
                datetime.now(UTC) - timedelta(minutes=idx * 5) if idx % 3 == 0 else None
            ),
...
class ScraperDataFactory:
    ...
    def create_opportunity_dict(idx: int = 0) -> Dict[str, Any]:
        ...
        return {
            ...
            "naics": ProspectFactory.NAICS_CODES[
                idx % len(ProspectFactory.NAICS_CODES)
            ],
            "value": f"${(idx + 1) * 15000:,}",
            "posted_date": (datetime.now(UTC).date() - timedelta(days=idx)).isoformat(),
            "response_date": (
                datetime.now(UTC).date() + timedelta(days=15 - idx)
            ).isoformat(),
