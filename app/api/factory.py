        ...
        def wrapped(*args, **kwargs):
            ...
            try:
                ...
                return error_response(
                    e.status_code, e.message, error_type="validation_error"
                )
            except NotFoundError as e:
                return error_response(e.status_code, e.message, error_type="not_found")
            except DatabaseError as e:
                local_logger.error(
                    f"Database error in {f.__name__}: {str(e)}", exc_info=True
                )
                return error_response(
                    e.status_code, e.message, error_type="database_error"
                )
            except ScraperError as e:
                local_logger.error(
                    f"Scraper error in {f.__name__}: {str(e)}", exc_info=True
                )
                return error_response(
                    e.status_code, e.message, error_type="scraper_error"
                )
            except AuthenticationError as e:
                return error_response(e.status_code, e.message, error_type="auth_error")
            except AuthorizationError as e:
                return error_response(
                    e.status_code, e.message, error_type="authorization_error"
                )
            except AppError as e:
                local_logger.error(
                    f"Application error in {f.__name__}: {str(e)}", exc_info=True
                )
                return error_response(e.status_code, e.message, error_type="app_error")
            except SQLAlchemyError as e:
                local_logger.error(
                    f"SQLAlchemy error in {f.__name__}: {str(e)}", exc_info=True
                )
                return error_response(
                    500, "Database error", error_type="database_error"
                )
            except Exception as e:
                local_logger.error(
                    f"Unhandled exception in {f.__name__}: {str(e)}", exc_info=True
                )
                return error_response(
                    500, "An unexpected error occurred", error_type="server_error"
                )

        # Register the route with the blueprint
        bp.add_url_rule(
            rule, endpoint=f.__name__, view_func=wrapped, methods=methods, **options
        )
        return wrapped
...
def _check_auth(level: str) -> Optional[tuple[dict, int]]:
    ...
    if level == "super_admin" and user_role != "super_admin":
        return (
            jsonify({"status": "error", "message": "Super admin access required"}),
            403,
        )
...
def error_response(
    status_code: int, message: str, error_type: Optional[str] = None, **kwargs
) -> tuple[dict, int]:
