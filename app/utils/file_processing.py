...
def update_processing_log(log: FileProcessingLog, success: bool, 
...
def validate_file_content(file_path: str, expected_columns: List[str] = None) -> Dict[str, Any]:
    """
...
def get_recent_files_for_source(source_id: int, limit: int = 10) -> List[FileProcessingLog]:
    """Get recent processing logs for a data source."""
    return FileProcessingLog.query.filter_by(source_id=source_id).order_by(
...
def cleanup_old_processing_logs(days_old: int = 90):
    ...
    deleted_count = db.session.query(FileProcessingLog).filter(
...
def analyze_file_processing_trends(source_id: int, days: int = 30) -> Dict[str, Any]:
    ...
    avg_records = sum(log.records_processed or 0 for log in logs if log.success) / success_count if success_count > 0 else 0
