...
def get_files_by_source(raw_data_path: Path) -> dict[str, list[tuple[Path, datetime]]]:
    ...
    for source_dir in raw_data_path.iterdir():
        ...
        for file_path in source_dir.iterdir():
            if file_path.is_file():
                ...
                if timestamp is not None:
                    ...
                    logger.warning(
                        f"Skipping file with invalid timestamp: {file_path.name}"
                    )
                    continue
        ...
        logger.info(
            f"Found {len(files_with_timestamps)} files for source: {source_name}"
        )
    ...
    for source_name, files_with_timestamps in files_by_source.items():
        ...
        if source_id:
            ...
            successful_file_paths = [
                log.file_path for log in recent_logs if log.success
            ]
            successful_files = [
                Path(fp) for fp in successful_file_paths if Path(fp).exists()
            ]
        ...
        files_to_keep = [
            (fp, ts) for fp, ts in files_with_timestamps if fp in files_to_preserve
        ]
        files_to_delete = [
            (fp, ts) for fp, ts in files_with_timestamps if fp not in files_to_preserve
        ]
        ...
        stats[source_name] = {
            ...
            "kept_successful": len(
                [f for f in successful_files if f in files_to_preserve]
            ),
        ...
        logger.info(
            f"Source '{source_name}': Keeping {kept_count} files "
        ...
        if successful_files:
            logger.info(
                f"  Protected successful files: {[f.name for f in successful_files]}"
            )
...
def cleanup_raw_data(
    ...
    stats = apply_retention_policy(
        files_by_source, retention_count=retention_count, dry_run=False
    )
...
def main():
    ...
    stats = apply_retention_policy(
        files_by_source, retention_count=args.retention_count, dry_run=dry_run
    )
    ...
    for source_name, source_stats in stats.items():
        logger.info(
            f"{source_name:35} | Kept: {source_stats['kept']:2} | "
