...
def cleanup_stuck_enhancements(max_age_hours=1):
    ...
    try:
        ...
        for prospect in stuck_prospects:
            logger.info(
                f"Resetting enhancement status for prospect {prospect.id}: {prospect.title[:50]}..."
            )
...
def cleanup_all_in_progress_enhancements():
    ...
    """
    try:
        in_progress_prospects = (
            db.session.query(Prospect)
            .filter(Prospect.enhancement_status == "in_progress")
            .all()
        )
        ...
        logger.info(
            f"Found {count} in-progress enhancement requests, resetting to idle..."
        )

        for prospect in in_progress_prospects:
            logger.info(
                f"Resetting prospect {prospect.id}: {prospect.title[:50] if prospect.title else 'No title'}..."
            )
            prospect.enhancement_status = "idle"
            prospect.enhancement_started_at = None
            prospect.enhancement_user_id = None
...
def get_enhancement_statistics():
    ...
    try:
        ...
        for status in ["idle", "in_progress", "failed"]:
            count = (
                db.session.query(Prospect)
                .filter(Prospect.enhancement_status == status)
                .count()
            )
            stats[status] = count
