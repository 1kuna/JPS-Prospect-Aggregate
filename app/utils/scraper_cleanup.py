...
def cleanup_stuck_scrapers(max_age_hours=2):
    ...
    try:
        ...
        stuck_scrapers = db.session.query(ScraperStatus).filter(
        ...
        for scraper_status in stuck_scrapers:
            ...
            source_name = data_source.name if data_source else f"Source ID {scraper_status.source_id}"
            
            logger.info(f"Resetting stuck scraper: {source_name} (stuck since {scraper_status.last_checked})")
...
def cleanup_all_working_scrapers():
    """
    Reset all scrapers that are currently marked as 'working' to 'failed'.
    
    This is useful when the server restarts and we want to reset all
    scraper statuses since in-memory processing state is lost.
    
    Returns:
        int: Number of scrapers that were reset
    """
    try:
        working_scrapers = db.session.query(ScraperStatus).filter(
            ScraperStatus.status == 'working'
        ).all()
        ...
        count = len(working_scrapers)
        logger.info(f"Found {count} working scrapers, resetting to failed...")
        
        for scraper_status in working_scrapers:
            data_source = db.session.query(DataSource).get(scraper_status.source_id)
            source_name = data_source.name if data_source else f"Source ID {scraper_status.source_id}"
            
            logger.info(f"Resetting scraper: {source_name}")
            scraper_status.status = 'failed'
            scraper_status.details = "Reset due to server restart - processing state was lost"
            scraper_status.last_checked = datetime.utcnow()
...
def get_scraper_statistics():
    ...
    try:
        ...
        for status in ['working', 'completed', 'failed']:
            count = db.session.query(ScraperStatus).filter(
        ...
        long_running = db.session.query(ScraperStatus).filter(
            ScraperStatus.status == 'working',
            ScraperStatus.last_checked < cutoff_time
        ).count()
        ...
        potentially_stuck = db.session.query(ScraperStatus).filter(
            ScraperStatus.status == 'working',
...
def is_scraper_stuck(source_id, max_age_hours=2):
    ...
    try:
        ...
        scraper_status = db.session.query(ScraperStatus).filter(
...
def reset_scraper_status(source_id, status='failed', details=None):
    ...
    try:
        ...
        scraper_status.details = details or f"Status reset from {old_status} to {status}"
        scraper_status.last_checked = datetime.utcnow()
