...
def map_universal_fields(record: Dict, source_file: str, source_mapping: Dict) -> Dict:
    ...
    if 'release_date_field' in source_mapping:
        mapped_data['release_date_raw'] = record.get(source_mapping['release_date_field'])
...
def create_prospect_from_data(mapped_data: Dict, source_id: int) -> Prospect:
    ...
    extra_data = {}
    core_fields = {
        'id', 'title', 'description', 'agency', 'estimated_value_text', 'source_file',
        'native_id', 'naics', 'contract_type', 'set_aside', 'place_city', 'place_state',
        'place_country', 'primary_contact_email', 'primary_contact_name', 'release_date_raw',
        'award_date_raw'
    }
...
def bulk_map_and_create_prospects(records: List[Dict], source_mapping: Dict, 
                                source_id: int, source_file: str) -> List[Prospect]:
    """
