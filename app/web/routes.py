...
def index(path):
    ...
    current_app.logger.info(
        f"Serving React SPA from {react_build_dir} for path: {path}"
    )

    # Check if the build directory exists
    if not os.path.exists(react_build_dir):
        current_app.logger.error(
            f"React build directory not found at {react_build_dir}"
        )
        return {
            "error": "React application not built. Please run 'npm run build' in the frontend-react directory."
        }, 500
    ...
    return {
        "error": "React application index not found. Please check the build directory."
    }, 500
