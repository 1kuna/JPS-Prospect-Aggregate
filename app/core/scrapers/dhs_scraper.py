...
class DHSForecastScraper(ConsolidatedScraperBase):
    ...
    def _custom_dhs_transforms(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'set_aside' in df.columns and 'small_business_program' in df.columns:
                df['consolidated_set_aside'] = df.apply(self._consolidate_set_aside_fields, axis=1)
                ...
                self.logger.debug("Consolidated DHS set-aside and small business program fields.")
    ...
    def _dhs_create_extras(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                if 'award_qtr_raw' in df.columns:
                    ...
                    if pd.notna(award_quarter) and str(award_quarter).strip():
                        ...
                        extras['award_date_note'] = "Date derived from fiscal quarter - represents tentative start of quarter"
                ...
                if 'small_business_program' in df.columns:
                    ...
                    if pd.notna(sb_program) and str(sb_program).strip() not in ['', 'None', 'N/A', 'TBD']:
                        extras['original_small_business_program'] = str(sb_program).strip()
            ...
            self.logger.debug(f"Created DHS extras JSON for {len(extras_data)} rows with enhanced metadata")
