...
from app.core.config_converter import create_hhs_config
...
class HHSForecastScraper(ConsolidatedScraperBase):
    ...
    async def hhs_setup(self) -> bool:
        ...
        if not success:
            ...
            success = await self.navigate_to_url(self.base_url, wait_until='networkidle')
            if not success:
        ...
        self.logger.info(f"Waiting for View All button: {self.config.pre_export_click_selector}")
        try:
            await self.wait_for_selector(self.config.pre_export_click_selector, timeout=30000)
        except Exception as e:
            self.logger.error(f"View All button not found: {e}")
            return False
        
        # Click "View All" button
        self.logger.info(f"Clicking 'View All' button: {self.config.pre_export_click_selector}")
