...
def check_enhancement_results():
    ...
    with app.app_context():
        ...
        print("="*80)
        print(f"Found {len(prospects_with_enhancement)} prospects with enhanced set-aside data")
        print()
        ...
        for i, prospect in enumerate(prospects_with_enhancement, 1):
            ...
            if (prospect.inferred_data.llm_confidence_scores and 
                'set_aside' in prospect.inferred_data.llm_confidence_scores):
        ...
        # Get some statistics
        total_with_set_aside = db.session.query(Prospect).filter(
            Prospect.set_aside.isnot(None),
            Prospect.set_aside != ''
        ).count()
