...
def create_app():
    ...
    app.config["SQLALCHEMY_BINDS"] = {"users": active_config.USER_DATABASE_URI}
...
def get_existing_file_logs():
    ...
    try:
        ...
        logs = (
            db.session.query(FileProcessingLog, DataSource)
            .join(DataSource, FileProcessingLog.source_id == DataSource.id)
            .all()
        )
...
def backfill_missing_logs(dry_run=True):
    ...
    data_sources = {
        source.name: source for source in db.session.query(DataSource).all()
    }
    ...
    for dir_name, files in files_by_source.items():
        ...
        if not source_name:
            logger.warning(
                f"Directory '{dir_name}' not mapped to any data source, skipping"
            )
            continue
            ...
            if dry_run:
                ...
                try:
                    ...
                    log = FileProcessingLog(
                        ...
                        processing_completed_at=file_info[
                            "timestamp"
                        ],  # Use file timestamp
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Backfill missing file processing logs"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
