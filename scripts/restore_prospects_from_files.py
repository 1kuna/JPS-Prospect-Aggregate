...
def find_most_recent_file(directory: Path, pattern: str) -> Optional[Path]:
...
def process_agency_data(agency_dir: str, scraper_key: str, config_key: str, source_id: int) -> bool:
    ...
    try:
        ...
        if 'id' not in df.columns:
            df['id'] = df.apply(lambda row: generate_prospect_id(row, source_id), axis=1)
        
        # Filter to only valid Prospect columns
        valid_columns = {
            'id', 'native_id', 'title', 'ai_enhanced_title', 'description', 'agency', 
            'naics', 'naics_description', 'naics_source', 'estimated_value', 'est_value_unit',
            'estimated_value_text', 'estimated_value_min', 'estimated_value_max', 
            'estimated_value_single', 'release_date', 'award_date', 'award_fiscal_year',
            'place_city', 'place_state', 'place_country', 'contract_type', 'set_aside',
            'set_aside_standardized', 'set_aside_standardized_label', 'primary_contact_email',
            'primary_contact_name', 'loaded_at', 'ollama_processed_at', 'ollama_model_version',
            'enhancement_status', 'enhancement_started_at', 'enhancement_user_id', 
            'extra', 'source_id'
        }
        ...
        logger.info(f"Inserting {len(df_filtered)} prospects from {most_recent_file.name}")
        bulk_upsert_prospects(df_filtered, preserve_ai_data=True, enable_smart_matching=False)
...
def restore_all_prospects():
    ...
    with app.app_context():
        ...
        # Process each agency
        for agency_dir in DIRECTORY_TO_SCRAPER_KEY.keys():
            ...
            success = process_agency_data(agency_dir, scraper_key, config_key, source.id)
        ...
        logger.info(f"Successfully processed: {success_count}/{total_agencies} agencies")
