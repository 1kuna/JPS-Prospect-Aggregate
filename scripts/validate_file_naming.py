...
class FileNamingValidator:
        ...
        for item in self.data_dir.iterdir():
            ...
            if dir_name.upper() in AGENCIES:
                result.valid_files.append(
                    f"Directory: {dir_name} (follows consolidated structure)"
                )
                continue

            # Any other directory is non-standard
            result.directory_issues.append(
                f"Non-standard directory: {dir_name} (should use agency abbreviation)"
            )
    ...
    def _validate_single_file(
        self, file_path: Path, dir_name: str, result: ValidationResult
    ):
        ...
        if not self.timestamp_pattern.search(file_name):
            result.invalid_files.append(
                f"{file_rel_path}: Missing timestamp pattern (YYYYMMDD_HHMMSS)"
            )
            return
        ...
        if not file_name.lower().startswith(
            dir_name.lower().replace("_", "").replace("-", "")
        ):
            result.naming_issues.append(
        ...
        if file_rel_path not in [
            issue.split(":")[0] for issue in result.invalid_files + result.naming_issues
        ]:
...
def main():
    ...
    total_issues = (
        ...
        + len(result.naming_issues)
        + len(result.directory_issues)
    )
