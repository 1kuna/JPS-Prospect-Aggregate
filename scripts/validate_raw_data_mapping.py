            ...
            if config.file_read_strategy == "html" or file_path.parent.name == "treas":
                ...
                try:
                    ...
                    if tables and len(tables) > 0:
                        ...
                        print(f"Successfully read Treasury HTML table with {len(df)} rows, {len(df.columns)} columns")
                    else:
...
def validate_mapping(source_name: str, config) -> Dict:
    ...
    result["file_date"] = datetime.fromtimestamp(latest_file.stat().st_mtime).strftime("%Y-%m-%d %H:%M")
    ...
    mapped_columns = set(config.raw_column_rename_map.keys()) if config.raw_column_rename_map else set()
...
def print_validation_report(results: List[Dict]):
    ...
    for result in results:
        ...
        if result["file_found"]:
            ...
            print(f"   Mappings: {result.get('mappings_found', 0)} found, {result.get('mappings_missing', 0)} missing")
            
            if result.get("sample_data"):
                print("   Sample mapped fields:")
                for raw_col, info in list(result["sample_data"].items())[:3]:
                    print(f"     • '{raw_col}' → '{info['maps_to']}'")
                    print(f"       Non-null: {info['non_null_count']:,} ({info['non_null_count']/result['total_rows']*100:.1f}%)")
