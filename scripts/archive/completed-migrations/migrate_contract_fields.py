...
def migrate_existing_naics_data():
    ...
    prospects = Prospect.query.filter(
        Prospect.naics.isnot(None), Prospect.naics_source.is_(None)
    ).all()
...
def migrate_value_text_from_extra():
    ...
    prospects = Prospect.query.filter(
        Prospect.estimated_value_text.is_(None), Prospect.extra.isnot(None)
    ).all()
...
def migrate_contact_info_to_extra():
    ...
    prospects = Prospect.query.filter(Prospect.extra.isnot(None)).all()
                ...
                if f"original_contact_{contact_type}" not in contacts:
                    for field in field_names:
                        if field in prospect.extra and prospect.extra[field]:
                            contacts[f"original_contact_{contact_type}"] = (
                                prospect.extra[field]
                            )
                            updated = True
                            break
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Migrate contract fields for existing prospects"
    )
    parser.add_argument(
        "--with-llm",
        action="store_true",
