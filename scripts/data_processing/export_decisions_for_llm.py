...
def export_decisions_to_jsonl(output_file: str, include_reasons_only: bool = False):
    ...
    with app.app_context():
        ...
        query = (
            db.session.query(GoNoGoDecision, Prospect)
            ...
            for decision, prospect in decisions:
                ...
                training_example = {
                    "messages": [
                        ...
                        {"role": "user", "content": create_prospect_prompt(prospect)},
                        {
                            "role": "assistant",
                    ...
                    "metadata": {
                        ...
                        "estimated_value": str(prospect.estimated_value_single)
        ...
        logger.info(
            f"Successfully exported {exported_count} training examples to {output_file}"
        )
...
def create_prospect_prompt(prospect: Prospect) -> str:
    ...
    if prospect.estimated_value_single:
        ...
        value_info.append(
            f"Estimated Value Range: ${prospect.estimated_value_min:,.2f} - ${prospect.estimated_value_max:,.2f}"
        )
    ...
    if prospect.release_date:
        prompt_parts.append(
            f"Release Date: {prospect.release_date.strftime('%Y-%m-%d')}"
        )
...
def export_decisions_to_csv(output_file: str):
    ...
    with app.app_context():
                ...
                writer.writerow(
                    {
                        ...
                        "release_date": prospect.release_date.isoformat()
                        if prospect.release_date
                        else None,
                        "award_date": prospect.award_date.isoformat()
                        if prospect.award_date
                        else None,
        ...
        logger.info(
            f"Successfully exported {len(decisions)} decisions to {output_file}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Export Go/No-Go decisions for LLM training"
    )
    parser.add_argument(
        "--format",
    ...
    parser.add_argument(
        "--reasons-only", action="store_true", help="Only export decisions with reasons"
    )
    parser.add_argument(
        "--filename-prefix",
        default="go_no_go_decisions",
    ...
    if args.format in ["jsonl", "both"]:
        jsonl_file = (
            output_dir / f"{args.filename_prefix}{reasons_suffix}_{timestamp}.jsonl"
        )
